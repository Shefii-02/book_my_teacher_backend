<form action="{{ route('company.requests.banner.update',$form_class->id) }}" method="POST">
    @csrf
    @method('PUT')
        ...
        <div class="mx-3">
            ...
            <div class="grid md:grid-cols-2 gap-2 text-sm">
                ...
                <div>
                    ...
                    <textarea rows="5" name="notes" class="font-medium border  form-control w-100 w-full text-gray-900">{{ $form_class->notes ?? '—' }}</textarea>
                </div>
        </div>
            ...
            <div class="flex flex-wrap gap-3">
                @foreach ($statusOptions as $value => $meta)
                    <label class="cursor-pointer flex gap-2 items-center">
                        <input id="{{ $value }}" type="radio" class="border" name="status" value="{{ $value }}" class=" peer"
