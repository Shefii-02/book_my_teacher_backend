    ...
    <nav>
        ...
        <ol class="flex flex-wrap pt-1 mr-12 bg-transparent rounded-lg sm:mr-16">
            ...
            <li class="text-sm pl-2 font-bold capitalize text-white before:float-left before:pr-2 before:text-white before:content-['/']"
                aria-current="page">Demo Class</li>
        </ol>
        <h6 class="mb-0 font-bold text-white capitalize">Demo Class</h6>
    </nav>
@endsection
            ...
            <div class="flex-none w-full max-w-full px-3">
                <div
                    ...
                    <div class="p-6 pb-0 mb-3 border-b border-b-transparent flex justify-between items-center">
                        <h6 class="leading-tight dark:text-white dark:opacity-80 text-slate-400">Demo Classes List</h6>
                        <a href="{{ route('company.demo-classes.create') }}"
                    ...
                    <div class="flex-auto px-0 pt-0 pb-2">
                        <div class="p-0 overflow-x-auto">
                            <table
                                ...
                                <tbody>
                                    @foreach ($demo_classes as $key => $demo_class)
                                        <tr>
                                            ...
                                            <td class="px-6 py-3 text-center font-bold text-xs uppercase align-middle bg-transparent border-b border-solid tracking-none text-slate-400 opacity-70">
                                                <div class="flex items-center">
                                                    @if ($demo_class->thumbnail_image)
                                                        <img src="{{ asset('storage/' . $demo_class->thumbnail_image) }}"
                                                            class="h-9 w-9 rounded mr-2" alt="thumb">
                                                    @endif
                                                </div>
                                                  <span
                                                        class="text-sm leading-tight dark:text-white dark:opacity-80 text-slate-400">{{ $demo_class->title }}</span>
                                            </td>
                                            <td
                                                class="px-6 py-3 text-center font-bold text-xs uppercase align-middle bg-transparent border-b border-solid tracking-none text-slate-400 opacity-70">
                                                {{ $demo_class->host?->name ?? 'N/A' }}</td>
                                            <td
                                                class="px-6 py-3 text-center font-bold text-xs uppercase align-middle bg-transparent border-b border-solid tracking-none text-slate-400 opacity-70">
                                                {{ $demo_class->provider?->name ?? 'N/A' }}</td>
                                            <td
                                                class="px-6 py-3 text-center font-bold text-xs uppercase align-middle bg-transparent border-b border-solid tracking-none text-slate-400 opacity-70">
                                                {{ $demo_class->created_at?->format('d M Y') }}</td>
                                            <td
                                                class="px-6 py-3 text-center font-bold text-xs uppercase align-middle bg-transparent border-b border-solid tracking-none text-slate-400 opacity-70">
                                                {{ $demo_class->started_at?->format('d M Y H:i') ?? '-' }}</td>
                                            <td
                                                class="px-6 py-3 text-center font-bold text-xs uppercase align-middle bg-transparent border-b border-solid tracking-none text-slate-400 opacity-70">
                                                {{ $demo_class->ended_at?->format('d M Y H:i') ?? '-' }}</td>
                                            <td
                                                class="px-6 py-3 text-center font-bold text-xs uppercase align-middle bg-transparent border-b border-solid tracking-none text-slate-400 opacity-70">
                                                {{ $demo_class->max_participants ?? '-' }}</td>
                                            <td class="p-2 text-center">
                                                <span
                                                    class="{{ $statusColors[$demo_class->status] ?? 'bg-emerald-500/50' }} px-2.5 py-1.5 text-xs rounded text-white">{{ ucfirst($demo_class->status) }}</span>
                                            </td>
                                            <td
                                                        ...
                                                        <li>
                                                            <form id="form_{{ $demo_class->id }}" class="m-0 p-0"
                                                                ...
                                                                <a role="button" href="javascript:;"
                                                                    ...
                                                                    onclick="confirmDelete({{ $demo_class->id }})">Delete</a>
