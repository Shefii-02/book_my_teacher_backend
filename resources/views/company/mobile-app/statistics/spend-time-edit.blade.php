<div class="">
    ...
    <form method="POST" action="{{ route('company.app.statistics-spend.update-with-type', ['statistics_spend' => $class['id'], 'type' => $class['type']]) }}">
        ...
        <div class="mt-4 border-0">
            <div class="card-body">
                ...
                {{-- Duration --}}
                <div class="mb-4">
                    <label class="form-label fw-semibold">Spend Duration (Minutes)</label>
                    <input type="number" name="spend_duration" class="form-control border"
                        placeholder="Enter duration in minutes" min="1"
                        value="{{ old('spend_duration', isset($statisticsSpend) && $statisticsSpend->spend_duration ? $statisticsSpend->spend_duration : 0) }}">
                    @error('spend_duration')
                ...
                <div class="mb-4">
                    ...
                    <div class="form-check form-check-inline">
                        <input class="form-check-input border" type="radio" name="status" value="pending"
                            {{ old('status', isset($statisticsSpend) && $statisticsSpend->status) == 'pending' ? 'checked' : 'checked' }}>
                        <label class="form-check-label text-warning fw-semibold">
                            Pending
                        </label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input border" type="radio" name="status" value="approved"
                            {{ old('status', isset($statisticsSpend) && ($statisticsSpend->status == 'approved')) ? 'checked' : '' }}>
                        <label class="form-check-label text-success fw-semibold">
                            Approved
                        </label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input border" type="radio" name="status" value="rejected"
                            {{ old('status', isset($statisticsSpend) && ($statisticsSpend->status == 'rejected')) ? 'checked' : '' }}>
