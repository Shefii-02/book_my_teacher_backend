...
<form method="POST" action="{{ route('company.courses.manage-conversation-update', $course->course_identity) }}">
    @csrf
    ...
    <div id="selectedUsers" class="flex flex-wrap gap-3 mb-6">
        ...
        @foreach ($conversation?->members ?? [] as $member)
            ...
            <div class="border p-2 flex gap-2 bg-gray-50" id="user_{{ $member->user->id }}">

                <input type="hidden" name="users[]" value="{{ $member->user_id }}">

                <img src="{{ $member->profile_image ?? asset('default-user.png') }}" class="w-8 h-8 rounded-full">
                ...
                <button type="button" onclick="removeUser({{ $member->user->id }})" class="text-red-500 ml-2">
                    ×
                </button>
...
<script>
    ...
    searchInput.addEventListener('keyup', function() {
        ...
        fetch(`{{ route('company.conversation.search.users') }}?q=${value}`)
            .then(res => res.json())
            .then(data => {
